...
(function ($) {
  $.fn.smartZoom = function (method) {
    ...
    var publicMethods = {
      ...
      init: function (options) {
        ...
        if (settings.mouseEnabled == true) {
          ...
          if (settings.scrollEnabled == true) {
            ...
            containerDiv.bind(
              "mousewheel.smartZoom DOMMouseScroll.smartZoom",
              containerMouseWheelHander
            );
          }
          if (
            settings.dblClickEnabled == true &&
        ...
        if (settings.initCallback != null)
          // call callback function after plugin initialization
      ...
      zoom: function (scaleToAdd, globalRequestedPosition, duration) {
        ...
        if (!globalRequestedPosition) {
          // use target center if globalRequestedPosition is not set
        ...
        var newGlobalX =
          targetRect.x -
          (positionGlobalXDiff * sizeRatio - positionGlobalXDiff); // apply the ratio to positionGlobalDiff and then find the final target point
        var newGlobalY =
          targetRect.y -
          (positionGlobalYDiff * sizeRatio - positionGlobalYDiff);
        var validPosition = getValidTargetElementPosition(
          newGlobalX,
          newGlobalY,
          newWidth,
          newHeight
        ...
        if (duration == null)
          // default effect duration is 700ms
          duration = 700;

        dispatchSmartZoomEvent(
          ESmartZoomEvent.ZOOM,
          ESmartZoomEvent.START,
          false
        );
        ...
        animate(
          ...
          duration,
          function () {
            // set the new position and size via the animation function
      ...
      pan: function (xToAdd, yToAdd, duration) {
        ...
        if (duration == null)
          // default pan duration is 700ms
          duration = 700;
        var currentPosition = targetElement.offset();

        var targetRect = getTargetRect();
        var validPosition = getValidTargetElementPosition(
          currentPosition.left + xToAdd,
          currentPosition.top + yToAdd,
          targetRect.width,
          targetRect.height
        ); // add the given numbers to the current coordinates and valid the result

        if (
          validPosition.x != currentPosition.left ||
          validPosition.y != currentPosition.top
        ) {
          // stop previous effect before make calculation
    ...
    if (publicMethods[method]) {
      // if the parameter is an existing method, then we call this method
      return publicMethods[method].apply(
        this,
        Array.prototype.slice.call(arguments, 1)
      );
    } else if (typeof method === "object" || !method) {
      // else if it's an object we initilize the plugin
      if (
        targetElement[0].tagName.toLowerCase() == "img" &&
        !targetElement[0].complete
      ) {
        // if the target is an image when wait for image loading before initialization
        targetElement.bind(
          "load.smartZoom",
      ...
      $.error(
        "Method " + method + " does not exist on e-smartzoom jquery plugin"
      );
    }
    ...
    function mouseWheelHandler(e, delta) {
      ...
      if (smartData.currentWheelDelta * delta < 0)
        // if current and delta have != sign we set 0 to go to other direction
        smartData.currentWheelDelta = 0;
      smartData.currentWheelDelta += delta; // increment delta zoom faster when the user use wheel again
      publicMethods.zoom(
        smartData.mouseWheelDeltaFactor * smartData.currentWheelDelta,
        { x: e.pageX, y: e.pageY }
      ); // 0.15
    }
    ...
    function updateMouseMoveCursor() {
      var smartData = targetElement.data("smartZoomData");
      if (
        smartData.settings.mouseMoveEnabled != true ||
        smartData.settings.moveCursorEnabled != true
      )
        return;
      var targetRect = getTargetRect();
      ...
      if (
        ...
        parseInt(smartData.adjustedPosInfos.scale * 100)
      )
        // multiply by 100 to resolve precision problem
        targetElement.css({ cursor: "move" });
      else targetElement.css({ cursor: "default" });
    }
    ...
    function mouseUpHandler(e) {
      ...
      if (smartData.mouseMoveForPan) {
        ...
        if (
          smartData.moveLastPosition.distance(smartData.moveCurrentPosition) > 4
        ) {
          // smooth the drag end when user move the mouse fast
          var interpolateP = smartData.moveLastPosition.interpolate(
            smartData.moveCurrentPosition,
            -4
          );
          moveOnMotion(interpolateP.x, interpolateP.y, 500, true);
        } else {
          moveOnMotion(
            smartData.moveLastPosition.x,
            smartData.moveLastPosition.y,
            0,
            true
          );
        }
    ...
    function touchStartHandler(e) {
      ...
      smartData.moveCurrentPosition = new Point(
        firstTouch.pageX,
        firstTouch.pageY
      ); // save the finger position on screen on touch start
      smartData.moveLastPosition = new Point(
        firstTouch.pageX,
        firstTouch.pageY
      );
      ...
      for (var i = 0; i < nbTouch; ++i) {
        ...
        smartData.touch.lastTouchPositionArr.push(
          new Point(currentTouch.pageX, currentTouch.pageY)
        );
      }
    }
    ...
    function touchMoveHandler(e) {
      ...
      if (
        nbTouch == 1 &&
        !smartData.touch.touchPinch &&
        smartData.settings.touchMoveEnabled == true
      ) {
        ...
        if (!smartData.touch.touchMove) {
          ...
          if (
            downLastPoint.distance(
              new Point(currentFirstTouchEv.pageX, currentFirstTouchEv.pageY)
            ) < 3
          ) {
            // check if user really had moved
            return;
          } else smartData.touch.touchMove = true;
        }
        moveOnMotion(
          currentFirstTouchEv.pageX,
        ...
        nbTouch == 2 &&
        !smartData.touch.touchMove &&
        smartData.settings.pinchEnabled == true
      ) {
        ...
        var currentP1 = new Point(
          currentFirstTouchEv.pageX,
          currentFirstTouchEv.pageY
        );
        var currentP2 = new Point(
          currentSecondTouchEv.pageX,
          currentSecondTouchEv.pageY
        );
        ...
        if (Math.abs(currentDistance) < 3)
          // finger pixel error (jump between in and out mode sometimes if i don't do this check)
          return;

        var middle = new Point(
          (currentP1.x + currentP2.x) / 2,
          (currentP1.y + currentP2.y) / 2
        ); // get the point between the two fingers
        ...
        var newZoomScale =
          (targetRect.width * newZoomValueToAdd) / originSize.width; // the new zoom scale
    ...
    function touchEndHandler(e) {
      ...
      if (nbTouchAtEnd == 0) {
        // unbind listeners if the user take off all his fingers
        $(document).unbind("touchmove.smartZoom");
        $(document).unbind("touchend.smartZoom");
      }
      var smartData = targetElement.data("smartZoomData");
      ...
      if (smartData.touch.touchPinch)
        // nothing to do for pinch behaviour in this function
        return;

      if (smartData.touch.touchMove) {
        // smooth motion at end if we are in drag mode
        if (
          smartData.moveLastPosition.distance(smartData.moveCurrentPosition) > 2
        ) {
          // smooth only if the user drag fast
          var interpolateP = smartData.moveLastPosition.interpolate(
            smartData.moveCurrentPosition,
            -4
          ); // the end smooth motion is calculate according to last finger motion
          moveOnMotion(interpolateP.x, interpolateP.y, 500, true);
        }
      } else {
        if (
          smartData.settings.dblTapEnabled == true &&
          smartData.touch.lastTouchEndTime != 0 &&
          new Date().getTime() - smartData.touch.lastTouchEndTime < 400
        ) {
    ...
    function moveOnMotion(xPos, yPos, duration, motionEnd) {
      ...
      var newMarginLeft =
        currentPosition.left + (xPos - smartData.moveCurrentPosition.x); // add current mouseX (orTouchX) position difference to the target position
      var newMarginTop =
        currentPosition.top + (yPos - smartData.moveCurrentPosition.y);
    ...
    function zoomOnDblClick(pageX, pageY) {
      ...
      if (
        currentScale.toFixed(2) > dblClickMaxScale.toFixed(2) ||
        Math.abs(dblClickMaxScale - currentScale) >
          Math.abs(currentScale - originalScale)
      ) {
    ...
    function stopAnim(userActionType) {
      ...
      if (smartData.transitionObject) {
        // if css transformation is surpported
        if (smartData.transitionObject.cssAnimHandler)
          // stop the transformation end handler if it exists
          targetElement.off(
            $.support.transition,
            smartData.transitionObject.cssAnimTimer
          );
        ...
        if (smartData.transitionObject.css3dSupported) {
          cssObject[smartData.transitionObject.transform] =
            "translate3d(" +
            targetRect.x +
            "px, " +
            targetRect.y +
     ...
     */
    function getValidTargetElementPosition(
      xPosition,
      yPosition,
      width,
      height
    ...
    function adjustToContainer() {
      ...
      var containerDivNewLeft = getContainerDivPositionFromSettings(
        smartData.settings.left,
        parentOffset.left,
        containerDiv.parent().width()
      );
      var containerDivNewTop = getContainerDivPositionFromSettings(
        smartData.settings.top,
        parentOffset.top,
      ...
      containerDiv.width(
        getContainerDivSizeFromSettings(
          smartData.settings.width,
          containerDiv.parent().width(),
          containerDivNewLeft - parentOffset.left
        )
      ); // apply size found to zoomablecontainer
      containerDiv.height(
        getContainerDivSizeFromSettings(
          smartData.settings.height,
          containerDiv.parent().height(),
          containerDivNewTop - parentOffset.top
        )
      );

      var containerRect = getRect(containerDiv); // get the rectangle from the new containerDiv position and size
      var scaleToFit = Math.min(
        Math.min(
          containerRect.width / originalSize.width,
          containerRect.height / originalSize.height
        ),
        1
      ).toFixed(2); // scale to use to include the target into containerRect
    ...
    function animate(
      ...
      if (smartData.transitionObject != null) {
        ...
        cssObject[smartData.transitionObject.transition] =
          "all " + duration / 1000 + "s ease-out"; // set effect duration

        if (smartData.transitionObject.css3dSupported)
          // use css 3d translate if supported
          cssObject[smartData.transitionObject.transform] =
            "translate3d(" +
            left +
            "px, " +
        ...
        if (callback != null) {
          ...
          target.one(
            ...
            smartData.transitionObject.cssAnimHandler
          );
        }
        target.css(cssObject); // apply css transformation
      } else {
        // use JQuery animate if css transition is not supported
    ...
    function getTargetRect(globalPosition) {
      ...
      if (smartData.transitionObject != null) {
        // if CSS3 transition is enabled
        var transformMatrix = targetElement.css(
          smartData.transitionObject.transform
        );
        if (
          transformMatrix &&
          transformMatrix != "" &&
          transformMatrix.search("matrix") != -1
        ) {
    ...
    function dispatchSmartZoomEvent(actionType, actionStep, stop) {
      ...
      if (stop == true && smartData.currentActionType != actionType) {
        ...
        eventTypeToDispatch =
          smartData.currentActionType + "_" + ESmartZoomEvent.END;
        ...
        if (
          ...
          eventTypeToDispatch =
            smartData.currentActionType + "_" + smartData.currentActionStep;
        } else if (
          smartData.currentActionType == actionType &&
    ...
    function getBrowserTransitionObject() {
      ...
      for (var i = 0; i < length; i++) {
        // for all kind of css transition we make a test
        if (bodyStyle[transitionTestArr[i]] != null) {
          ...
          div.empty().remove();
          if (css3dSupported) {
            // return the kind of transformation supported
            return {
     ...
     */
    function getContainerDivSizeFromSettings(
      settingsValue,
      ...
      if (settingsValue.search && settingsValue.search("%") != -1)
        return (
          (zoomableContainerParentValue - divPosDiff) *
          (parseInt(settingsValue) / 100)
        );
     ...
     */
    function getContainerDivPositionFromSettings(
      settingsValue,
    ...
    function Point(x, y) {
      ...
      this.interpolate = function (pointToInterpolate, percentInterpolate) {
        var x =
          percentInterpolate * this.x +
          (1 - percentInterpolate) * pointToInterpolate.x;
        var y =
          percentInterpolate * this.y +
          (1 - percentInterpolate) * pointToInterpolate.y;
      ...
      this.distance = function (point) {
        return Math.sqrt(
          Math.pow(point.y - this.y, 2) + Math.pow(point.x - this.x, 2)
        );
      };
    }
  };
